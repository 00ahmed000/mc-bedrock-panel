...
def parse_raw_properties() -> Dict[str, str]:
    """Read server.properties into a flat {key: rawvalue} dict; comments and blank lines dropped."""
    raw: Dict[str, str] = {}
    if not os.path.exists(config.PROPERTIES_PATH):
        return raw
    with open(config.PROPERTIES_PATH, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
...
def get_properties() -> FullServerProperties:
    """
    Return the current server.properties, typed and validated. Any single
...
def save_properties(props: FullServerProperties) -> None:
    ...
    os.makedirs(os.path.dirname(config.PROPERTIES_PATH), exist_ok=True)
    tmp_path = config.PROPERTIES_PATH + ".tmp"
    with open(tmp_path, "w", encoding="utf-8") as f:
        f.writelines(f"{k}={v}\n" for k, v in raw.items())
    os.replace(tmp_path, config.PROPERTIES_PATH)
    fsutil.set_bedrock_ownership(config.PROPERTIES_PATH)
